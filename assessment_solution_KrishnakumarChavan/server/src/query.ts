...
// Please run sql queries given in all.txt before running query.ts
...
(async () => {
	try {
		...
		console.log('Recognition deleted:', delete_recognition);

		// This acts like self recognition delete
		const delete_recognition_2 = await deleteRecognitionById(5, 3, 'TEAMLEAD');
		console.log('Recognition deleted:', delete_recognition_2);
	} catch (error) {
		console.error('Error:', error);
	}
})();
